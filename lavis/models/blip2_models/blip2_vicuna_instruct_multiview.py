...
@registry.register_model("blip2_vicuna_instruct_multiview")
class Blip2VicunaInstructMultiview(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    def __init__(
        ...
        self.tokenizer = self.init_tokenizer_original(truncation_side="left")
        ...
        self.llm_model = LlamaForCausalLM.from_pretrained(
            llm_model, torch_dtype=torch.float16, load_in_8bit=True
        )
    ...
    def forward(self, samples):
        ...
        image = samples["vfeats"]

        B = image.shape[0]
        T = image.shape[1]  # time dimension
        V = image.shape[2]  # view dimension
        
        with self.maybe_autocast():
            # image_embeds = self.ln_vision(self.visual_encoder(image))
            if image.dim() == 6 and V == 1:  # Single view case
                tmp_list = []
                for t in range(T):
                    curr_images = image[:, t, 0, ...].squeeze(1).squeeze(1)
                ...
                for t in range(T):
                    view_list = []
                    for v in range(V):
                        curr_images = image[:, t, v, ...].squeeze(1).squeeze(1) #8, 3, 364， 364
                        view_embeds = self.ln_vision(
                            self.visual_encoder(curr_images))  # Squeeze view dimension 8 677 1408
                        view_embeds = self.slot_attention(view_embeds) # 8 32 1408
                        positional_embedding = self.view_pos_embedding[v].unsqueeze(0).unsqueeze(1) #1 1 1408
                        view_embeds = view_embeds + positional_embedding
                        view_list.append(view_embeds.unsqueeze(1))
                    time_embeds = torch.cat(view_list, dim=1)  # Concatenate views 8 6 32 1408
                    tmp_list.append(time_embeds.unsqueeze(1)) #8 1 6 32 1408
                image_embeds = torch.cat(tmp_list, dim=1) #8 20 6 32 1408
                image_embeds = image_embeds.view(B, -1, image_embeds.shape[-1]) #8 3840 1408 # Flatten time and views
        ...
        text_output_tokens = self.llm_tokenizer(
            [t + self.llm_tokenizer.eos_token for t in samples["answers"]],
            return_tensors="pt",
            padding="longest",
            truncation=True,
        ...
        if prompt:
            if prompt.count("{}") == 2:
                if 'ocr_tokens' in samples:
                    text_input = [
                        prompt.format(', '.join(samples['ocr_tokens'][i][:30]), samples["questions"][i])
                    for i in range(len(samples["questions"]))]
                elif 'choices' in samples:
                    text_input = []
                    for i in range(len(samples["questions"])):
                        this_choices = [f"({string.ascii_lowercase[j]}) {ch}" for j, ch in enumerate(samples["choices"][i])]
                        this_choices = " ".join(this_choices)
                        text_input.append(prompt.format(samples["questions"][i], this_choices))
            else:
                text_input = [prompt.format(question) for question in samples["questions"]]
        else:
            text_input = samples["questions"]

        samples["questions"] = text_input
