...
@registry.register_model("blip2_opt_multiview_new")
class Blip2OPTMultiviewNew(Blip2Base):
    """
    BLIP2 OPT model.
    ...
    def forward(self, samples):
        ...
        text = [self.prompt.format(q, a) + "\n" for q, a in zip(samples["questions"], samples["answers"])]
        if self.check:
            print(text)
            self.check = False
