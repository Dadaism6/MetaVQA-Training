...
@registry.register_model("blip2_vqa_t5_elm_multiview")
class Blip2VQAT5ELMMulti(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        ...
        "metavqa_elm_mixmulti": "configs/models/blip2/blip2_metavqa_elm_mixmulti.yaml",
    }
        ...
        with self.maybe_autocast():
            if vfeats.dim() == 6 and V == 1:  # Single view case
                ...
                for t in range(T):
                    view_list = []
                    for v in range(V):
                        curr_images = vfeats[:, t, v, ...].squeeze(1).squeeze(1) #8, 3, 364， 364
            ...
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(
                device)  # [B, num_views * num_timesteps * height * width] #8 3840

            query_tokens = torch.cat([self.query_tokens, self.extra_query_tokens], dim=1) #1 128 768
            query_tokens = query_tokens.expand(image_embeds.shape[0], -1, -1) #8 128 768
            ...
            t5_query = query_output.last_hidden_state  # [6, 128, 768]
    ...
    def generate(
        ...
        """
        print("Warning!!!!!!!!!!!!!!!!!!!!!!!!!!!Get into Generate function")
        device = samples["vfeats"].device
