...
def process_image_path(annotation_val):
    ...
    return image_path
def process_image_path_real(annotation_val):
    image_path = []
    for view in ["front", "leftf", "leftb", "rightf", "rightb", "back"]:
        if view in annotation_val['real']:
            curr_view_path = []
            for path in annotation_val['real'][view]:
                curr_view_path.append(path)
            if len(curr_view_path) == 4:
                curr_view_path.insert(0, curr_view_path[0])#repeat the first image
            image_path.append(curr_view_path)
        else:
            raise ValueError(f"The view {view} is not in the annotation")
    # we should have equal number of images for each view
    for i in range(1, len(image_path)):
        if len(image_path[i]) != len(image_path[0]):
            raise ValueError(f"The number of images in each view should be the same")
    return image_path
class EvalMetavqaRealDataset(VQADataset):
    def __init__(self, vis_processor=None, text_processor=None, vis_root=None, ann_paths=None):
        ...
        print("Eval Nuscenes Critical Training dataset: The number of Training data: ", len(self.questions))
    def init_metavqa(self, ann_paths):
        ann_paths = [
                ...
                #      "/path/to/your/local/drive/metavqa_final/vqa/training/safety_critical_processed/Waymo",
                     "/path/to/your/local/drive/metavqa_final/vqa/training/safety_critical_processed/CAT"]
        # sample_numbers = [2000, 2000, 1000, 1000]
            ...
            for ann_file in ann_files:
                if ann_file.endswith('.json'):
                    ...
                    while count < sample_number and index < total_annotations:
                        ...
                        val = annotations[id]
                        if "NuScenes_Mixed" in ann_path:
                            image_path = process_image_path_real(val)
                            self.dataset_source.append("real")
                        else:
    ...
    def __getitem__(self, index):
        ...
        if dataset_source == "real":
            image = self.load_and_process_images_real(image_path)
        else:
    ...
    def load_and_process_images_real(self, images_paths):
        # Define the views
        views = ["front", "leftf", "leftb", "rightf", "rightb", "back"]
...
class EvalMetavqaRealEvalDataset(VQADataset):
    def __init__(self, vis_processor=None, text_processor=None, vis_root=None, ann_paths=None):
        """
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
        ...
        self.init_metavqa(ann_paths)
        print("Eval Nuscenes Eval Critical:The number of Validation data: ", len(self.questions))
    def init_metavqa(self, ann_paths):
        ...
        ann_paths = ["/path/to/your/local/drive/metavqa_final/vqa/NuScenes_Mixed/validation/multi_frame_processed",
                     "/path/to/your/local/drive/metavqa_final/vqa/NuScenes_Mixed/validation/single_frame_processed"]
        sample_numbers = [5000, 5000]
        # sample_numbers = [5,5,5]
        # sample_numbers = [200, 200, 100, 100]
        # sample_numbers = [10, 10, 5, 5]
        # sample_number = 2000
        # sample_number = 5
        for ann_path, sample_number in zip(ann_paths,sample_numbers):
            print("Load from: " + ann_path)
    ...
    def load_and_process_images_real(self, images_paths):
        ...
        assert processed_images_tensor.shape[0] == 5, f"First dimension is not 5, but {processed_images_tensor.shape[0]}"
        assert processed_images_tensor.shape[1] == 6, f"Second dimension is not 6, but {processed_images_tensor.shape[1]}"
        return processed_images_tensor
