...
def process_image_path_real(annotation_val):
    ...
    for i in range(1, len(image_path)):
        if len(image_path[i]) != len(image_path[0]):
            raise ValueError(f"The number of images in each view should be the same")
    return image_path
class MetavqaRealDataset(VQADataset):
    def __init__(self, vis_processor=None, text_processor=None, vis_root=None, ann_paths=None):
        ...
        print("Real dataset Training dataset: The number of Training data: ", len(self.questions))
    def init_metavqa(self, ann_paths):
        # ann_paths = ['/path/to/your/local/drive/dataset/nuscene_real_train/',
        #              '/path/to/your/local/drive/dataset/waymo_train/']
        # sample_numbers = [32000,32000]
        ann_paths = ['/path/to/your/local/drive/dataset/nuscene_real_train/']
        sample_numbers = [32000]
...
class MetavqaRealEvalDataset(VQADataset):
    def __init__(self, vis_processor=None, text_processor=None, vis_root=None, ann_paths=None):
        """
        vis_root (string): Root directory of images (e.g. coco/images/)
        ann_root (string): directory to store the annotation file
